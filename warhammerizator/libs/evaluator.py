...
class Evaluator:
    def __init__(self, cycleGAN, settings: Settings, experiment):
        ...
        if self.settings.trainer["bertscore"]: self.bertscore = evaluate.load('bertscore')
        ...
        if metric_name in ['bleu']:
            for pred, ref in zip(predictions, references):
                if metric_name == 'bleu':
                    ...
                    res = self.bertscore.compute(predictions=[pred], references=[ref], lang=self.settings.dataset["lang"])
    ...
    def __compute_classif_metrics__(self, pred_A, pred_B):
        ...
        for i in range(0, len(pred_A), self.settings.data_loader["batch_size"]):
            batch_a = pred_A[i:i + self.settings.data_loader["batch_size"]]
            inputs = classifier_tokenizer(batch_a, truncation=truncation, padding=padding,
                                          max_length=self.settings.dataset["max_sequence_length"], return_tensors="pt")
            inputs = inputs.to(device)
            with torch.no_grad():
                output = classifier(**inputs)
            y_pred.extend(np.argmax(output.logits.cpu().to(torch.float32).numpy(), axis=1))
        for i in range(0, len(pred_B), self.settings.data_loader["batch_size"]):
            batch_b = pred_B[i:i + self.settings.data_loader["batch_size"]]
            inputs = classifier_tokenizer(batch_b, truncation=truncation, padding=padding,
                                          max_length=self.settings.dataset["max_sequence_length"], return_tensors="pt")
            inputs = inputs.to(device)
            with torch.no_grad():
                output = classifier(**inputs)
    ...
    def run_eval_mono(self, epoch, current_training_step, phase, mono_dl_a_eval, mono_dl_b_eval):
        ...
        # avg_AB_r1_self, avg_AB_r2_self, avg_AB_rL_self = np.mean(scores_AB_r1_self), np.mean(
        #     scores_AB_r2_self), np.mean(scores_AB_rL_self)
        ...
        # avg_BA_r1_self, avg_BA_r2_self, avg_BA_rL_self = np.mean(scores_BA_r1_self), np.mean(
        #     scores_BA_r2_self), np.mean(scores_BA_rL_self)
        ...
        if phase == 'validation':
            ...
            if self.settings.trainer["eval_strategy"] == 'epochs':
                ...
                suffix = f'step{current_training_step}'
        else:
            if self.settings.trainer["from_pretrained"] is not None:
                if self.settings.trainer["save_base_folder"] is not None:
                    base_path = self.settings.trainer["save_base_folder"]
                else:
                    base_path = self.settings.trainer["save_base_folder"] / f"epoch_{epoch}/"
            else:
                base_path = self.settings.trainer["save_base_folder"] / "test" / f"/epoch_{epoch}/"
            suffix = f'epoch{epoch}_test'
        base_path.mkdir(parents=True, exist_ok=True)
        with open(base_path /  f"metrics_{suffix}.pickle", 'wb') as fp:
        ...
        if self.experiment:
            for m, v in metrics.items():
                mlflow.log_metric(f"{phase}_{m}", v, step=current_training_step)
    ...
    def run_eval_ref(self, epoch, current_training_step, phase, parallel_dl_evalAB, parallel_dl_evalBA):
        ...
        for batch in parallel_dl_evalAB:
            ...
            if self.args.bertscore:
                scores_AB_bscore.extend(self.__compute_metric__(transferred, references_b, 'bertscore'))
            else:
        ...
        avg_AB_r1_self, avg_AB_r2_self, avg_AB_rL_self = np.mean(scores_AB_r1_self), np.mean(
            scores_AB_r2_self), np.mean(scores_AB_rL_self)
        avg_AB_r1_ref, avg_AB_r2_ref, avg_AB_rL_ref = np.mean(scores_AB_r1_ref), np.mean(scores_AB_r2_ref), np.mean(
            scores_AB_rL_ref)
        ...
        for batch in parallel_dl_evalBA:
            ...
            if self.settings.trainer["bertscore"]:
                scores_BA_bscore.extend(self.__compute_metric__(transferred, references_a, 'bertscore'))
            else:
        ...
        avg_BA_r1_self, avg_BA_r2_self, avg_BA_rL_self = np.mean(scores_BA_r1_self), np.mean(
            scores_BA_r2_self), np.mean(scores_BA_rL_self)
        avg_BA_r1_ref, avg_BA_r2_ref, avg_BA_rL_ref = np.mean(scores_BA_r1_ref), np.mean(scores_BA_r2_ref), np.mean(
            scores_BA_rL_ref)
        ...
        if phase == 'validation':
            ...
            if self.settings.trainer["from_pretrained"] is not None:
                if self.settings.trainer["save_base_folder"] is not None:
                    ...
                    base_path = self.settings.trainer["from_pretrained"] / f"epoch_{epoch}"
            else:
                base_path = self.settings.trainer["save_base_folder"] / "test" / f"epoch_{epoch}"
            suffix = f'epoch{epoch}_test'
            # if self.settings.trainer["from_pretrained"] and 'GYAFCfm' in self.settings.trainer["from_pretrained"].as_posix():
            #     if 'family' in self.args.path_paral_test_ref:
            #         ds = 'family'
            #     elif 'music' in self.args.path_paral_test_ref:
            #         ds = 'music'
            #     suffix += f'_{ds}'
        os.makedirs(os.path.dirname(base_path), exist_ok=True)
        pickle.dump(metrics, open(base_path / f"metrics_{suffix}.pickle", 'wb'))
    ...
    def dummy_bscore(self):
        ...
        references = [
            ['i just left this car wash and was very satisfied !', 'i just left the car wash and i feel very satisfied',
             'i did not leave this car wash and was very satisfied',
             'i just left the car wash and i was very satisfied'],
            ["i would recommend ordering something once you 're seated", "ordering anything if you 're seated",
             'the ordering service is nice', 'the staffs here are very attentive'],
            ['they addressed all the broken items', 'one correct thing after another they care to address',
             'one broken thing after another they really care to address', 'they have good after sales service'],
            ['food was hot ( and fresh ) , i had the ribs .', "food was n't cold ( well cooked ) , i had the ribs",
             'the food is food , i had the ribs', 'food was warm , i had the ribs .'],
            ['i want to give this a 5 out of 5 star rating .', '5 stars is what i would give', 'give five stars to him',
             'give 5+ stars to him'],
            ['i would give an extra star if it allowed me', 'if i could give more stars , i definitely would',
             'if i could give more stars , i would', 'i would give you more stars if i could'],
            ['they will tell you though .', 'they will tell you the details', 'they will tell you though .',
             'they will tell you'],
            ['she seemed happy to be there', 'she was cheerful being there .', 'she was happy because being here',
             'she was happy being there']]
        scores = []
        for pred, ref in zip(predictions, references):
            res = self.bertscore.compute(predictions=[pred], references=[ref], lang=self.settings.dataset["lang"])
